...
export class JobHistoryMySuffixService {
    ...
    search(req?: any): Observable<HttpResponse<JobHistoryMySuffix[]>> {
        const options = createRequestOption(req);
        return this.http.get<JobHistoryMySuffix[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<JobHistoryMySuffix[]>) => this.convertArrayResponse(res));
    }
